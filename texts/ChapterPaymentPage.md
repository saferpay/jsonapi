...
><i class="glyphicon glyphicon-hand-right"></i> **Important** The payer might modify the return address (e.g. replace fail url with success url). We therefore recommend to always validate the outcome of the transaction and transaction related information by calling PaymentPage Assert with the corresponding token. 
Depending on the provider, the transaction status returned is either AUTHORIZED (reservation), or CAPTURED (booked). Thus even if you do not perform a subsequent Capture Request, the payment might be booked (captured) automatically. Please view the  <a target="_blank" href="http://saferpay.github.io/jsonapi/index.html#ChapterBatch ">Payment Method Feature matrix</a>to check which paymentmethods are captured automatically and therefore do have to be captured seperately.
